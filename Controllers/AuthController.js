...
export const forgotpass_reqOTP = async (req, res) => {
  const { email } = req.body;
  const existinguser = await prisma.user.findUnique({
    where: { email: email },
  });
  if (!existinguser) {
    return res.json({ success: false, message: "E-mail not registered." });
  }
  try {
    await SendOTP(email);
...
export const requestOTP = async (req, res) => {
  ...
  if (existinguser)
    return res.json({ success: false, message: "E-mail Already Registered" });
  try {
    await SendOTP(email);
    res.json({ success: true, message: "OTP sent" });
    expiresAt = Date.now() + 5 * 60 * 1000;
...
export const Signup = async (req, res, next) => {
  try {
    ...
    const user = await prisma.user.create({
      data: { username, email, fullname, password, admin },
    });
